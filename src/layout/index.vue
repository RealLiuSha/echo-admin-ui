<template>
    <div class="layout">
        <div id="app-main" :style="{'--real-sidebar-width': realSidebarWidth}">
            <header v-if="$store.state.settings.mode == 'pc'">
                <div class="header-container">
                    <div class="main">
                        <Logo />
                        <!-- 当头部导航大于 1 个的时候才会显示 -->
                        <div v-if="routesLength > 1" class="nav">
                            <template v-for="(item, index) in $store.state.menu.noChildrenRoutes">
                                <div v-if="item.hidden === false" :key="`noChild-${index}`" :class="{'item': true}">
                                    <svg-icon v-if="item.meta.icon" :name="item.meta.icon" />

                                    <el-tooltip class="item" effect="dark" content="该菜单尚未配置任何子级菜单" placement="bottom">
                            ...
                            <template v-for="(item, index) in $store.state.menu.routes">
                                <div v-if="item.children && item.children.length > 0 && item.hidden === false" :key="index" :class="{
                                         'item': true,
            ...
            <div class="wrapper">
                ...
                >
                    ...
                    >
                        <Logo
                            :show-logo="routesLength <= 1"
                            :class="{
                                'sidebar-logo': true,
                                'sidebar-logo-bg': routesLength <= 1,
                        ...
                        <el-menu
                            ...
                            :collapse-transition="false" :class="{
                                'is-collapse-without-logo': routesLength > 1 && $store.state.settings.mode == 'pc' && $store.state.settings.sidebarCollapse
                            }"
                        >
                            <transition-group name="sidebar">
                                <template v-for="route in $store.getters['menu/sidebarRoutes']">
                                    <SidebarItem v-if="route.hidden === false" :key="route.path" :item="route" :base-path="route.path" />
                                </template>
                            </transition-group>
                        </el-menu>
                    </div>
...
export default {
    ...
    computed: {
        ...
        routesLength() {
            return this.$store.state.menu.routes.length + this.$store.state.menu.noChildrenRoutes.length
        },
        realSidebarWidth() {
